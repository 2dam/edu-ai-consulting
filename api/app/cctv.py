...
def fetch_cctv(min_x: float, min_y: float, max_x: float, max_y: float, cctv_type: int = 1) -> list[dict]:
    ...
    try:
        ...
        logger.error("ITS CCTV 요청 실패: %s", exc)
        return []
    except ValueError:
        logger.error("ITS CCTV 응답이 JSON이 아님 (status=%s): %s", res.status_code, res.text[:300])
        return []

    raw_items = payload.get("response", {}).get("data", [])
    if not raw_items:
        logger.warning("ITS CCTV 응답에 data 없음: %s", str(payload)[:300])
